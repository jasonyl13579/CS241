...
ssize_t minixfs_write(file_system *fs, const char *path, const void *buf,
	...
	off_t offset = *off;
	int block_count = 1;
	block_count += offset / sizeof(data_block);
	...
	int start = 1;
	if ( (size_t)current_block > NUM_DIRECT_INODES + NUM_INDIRECT_INODES){
		errno = ENOSPC;
		return -1;
	}
	//direct block
	while (current_block < NUM_DIRECT_INODES){
		...
		off_t num = MIN((off_t)sizeof(data_block), remain_to_write);
