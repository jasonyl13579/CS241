...
uint32_t syscall_mmap(uint32_t length, int prot_flags, int flags,
	...
	if (length % PAGE_SIZE != 0 || offset % PAGE_SIZE != 0) return MAP_FAILED;
	
	struct mapped_file* file = NULL;
	if (pathname != NULL && (flags & MAP_ANONYMOUS) == 0){
		file = obtain_mapped_file(pathname);
		if (file == NULL) return MAP_FAILED;
	}
	struct vm_area* new_area = NULL;
	list_rev_for_each(current->stack_segment, current->area_list, area_prev, area_next, elem)
	{
		//printf("%08x-%08x\n", elem->start_addr, elem->end_addr);
		struct vm_area *pre_vma = list_prev(elem, area_prev, area_next);
		//printf("pre:%08x-%08x\n", pre_vma->start_addr, pre_vma->end_addr);
		if (elem->start_addr - pre_vma->end_addr > length){
			new_area = create_vm_area(current, elem->start_addr - length, elem->start_addr);
			list_link_before(new_area, elem->area_prev, area_prev, area_next);
			break;
		}
	}
	if (new_area == NULL) return MAP_FAILED;
	if (file != NULL){
		new_area->page_offset = offset;
		file->reference_count += offset;
		if (flags & MAP_SHARED){
			list_link_before(new_area, &file->shared_vma_list, area_prev, area_next)
		}
		new_area->file = file;	
	}
	/*list_rev_for_each(current->stack_segment, current->area_list, area_prev, area_next, elem)
	...
	printf("=================\nnew:%08x-%08x\n", new_area->start_addr, new_area->end_addr);*/
	if (new_area == NULL)
		return MAP_FAILED;
	if (prot_flags & PROT_READ) new_area->prot_read = 1;
	if (prot_flags & PROT_WRITE)new_area->prot_write = 1;
	if (prot_flags & PROT_EXEC) new_area->prot_exec = 1;
	if (flags & MAP_PRIVATE) new_area->map_private = 1;
	if (flags & MAP_ANONYMOUS) new_area->map_anonymous = 1;
	return new_area->start_addr;
}
...
addr32 syscall_sbrk(uint32_t shift) {
	if (shift % PAGE_SIZE != 0) return MAP_FAILED;
	if (current->heap_segment->end_addr + shift > current->stack_segment->start_addr) return MAP_FAILED;
	current->heap_segment->end_addr += shift;
	addr32 r = current->heap_segment->end_addr;
    return r;
}
