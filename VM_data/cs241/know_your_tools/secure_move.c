...
const static int NO_FILE_RETURN_CODE = 8;
...
int main(int argc, char *argv[]) {
	...
	static int sta = 1;	
	char* abs = "abs";
	char *heap = malloc(4*sizeof(char));
    char *file_name = argv[1];
    char *dest = argv[2];
    file *input_file = read_wrap(file_name);
	if (input_file == NULL) return  NO_FILE_RETURN_CODE;
    shred(file_name);
	...
	print_heap_addr(heap);
	print_static_variable_addr(&sta);
	print_string_literal_addr(abs);
	print_main(&main);
