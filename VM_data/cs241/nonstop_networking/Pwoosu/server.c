		...
		switch (metadata->command){
			case GET:
				if (metadata->size == 0){
					...
					metadata->size = size;
					metadata->remain = size;
					count = write_message_size(size, metadata->fd);
					if (count == -1) return count;
				}
				count = handle_get_write(event);
				if (count == -1 || count == 0) return count;
				break;
			case LIST:
				LOG("LIST");		
				...
				for(size_t i=0; i<vector_size(vector_file); i++){
					...
					if (i != vector_size(vector_file)-1)
						strcat(buffer, "\n");
				}
...
int handle_get_write(struct epoll_event event){
	...
	ssize_t offset = size - metadata->remain;
	ssize_t len = metadata->remain;
	int first = 0;
	if (metadata->remain > MAX_CHUNK_SIZE) len = MAX_CHUNK_SIZE;
	if (metadata->remain == size) {
		first = 1;
	}
	char* file_path = malloc(strlen(server_path)+strlen(metadata->filename)+2);
    sprintf(file_path,"%s/%s", server_path, metadata->filename);
...
int handle_put_read(struct epoll_event event){
	...
	while (1){
		...
		if (receive_size == -1 || receive_size == 0){	
			if (f != NULL) fclose(f);
			if (receive_size == 0 || (receive_size == -1 && (errno != EAGAIN || errno != EWOULDBLOCK))) {
		...
		if (receive_size >= len+1){
			...
			offset += receive_size;
			LOG("remain:%zd",metadata->remain);
			LOG("Offset:%ld\n", offset);
			fseek(f, 0, SEEK_END);
			ssize_t r = fwrite(buf, receive_size, 1, f);
			if (r < 0) perror("fwrite");
			//memcpy(metadata->filecontent + offset, buf, receive_size);
			if (metadata->remain == 0){
				LOG("Finish put:%s", server_path);
				int find = 0;
				metadata->type = OK;
				event.events = EPOLLOUT;
				epoll_ctl(efd, EPOLL_CTL_MOD, metadata->fd, &event);
