	...
	switch (command){		
		case GET:
			...
			if (size > 0){
				...
				size_t receive_size = read_all_from_socket(sock_fd, list, size+1); // extra one bit to test received_too_much_data
				list[size] = '\0';
				
				//printf("l:%s\n",list);
				if (receive_size == size+1){
					//char temp;
					//if (read_all_from_socket(sock_fd, &temp, 1)){
				...
				LOG("Received %zd bytes from server",receive_size);			
				free(list);
			}
			break;
		case PUT:
		...
		case LIST:
			...
			if (size > 0){
				...
				size_t receive_size = read_all_from_socket(sock_fd, list, size+1);
				list[size] = '\0';	
				if (receive_size == size+1){
					//char temp;
					//if (read_all_from_socket(sock_fd, &temp, 1))
				...
				printf("%s",list);
				fflush(stdout);
				LOG("Received %zd bytes from server",size);
				free(list);
			}
			break;
		case DELETE:
